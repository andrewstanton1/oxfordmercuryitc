...
class temperatureController:
    ...
    def getSignal(self, device, signal):
        try:
            ...
            pass
        self.ans = self.readValue(self.devices[device] + ":SIG:" + signal)  # .decode().split(":")[-1]
        return [device, self.ans]

    def getMaxVoltage(self, device):
        try:
            self.max_voltage = float(self.readValue(self.devices[device] + ":VLIM"))
        except:
            return [device, str(self.max_voltage)]
    ...
    def getResistance(self, device):
        try:
            ...
            return [device, str(self.resistance)]
    ...
    def setMaxVoltage(self, value, device):
        return self.setValue(device + ":VLIM:" + str(value))

    def setResistance(self, value, device):
        return self.setValue(device + ":RES:" + str(value))
    ...
    def setSetPointSetting(self, value, device):
        return self.setValue(device + ":LOOP:SWMD:" + str(value))
